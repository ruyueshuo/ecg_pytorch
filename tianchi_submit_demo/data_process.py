...
def split_data(file2idx, val_ratio=0.1,config=None):
    '''
...
def count_labels(data, file2idx, config=None):
    '''
    统计每个类别的样本数
    :param data:
    :param file2idx:
...
def train(name2idx, idx2name,config=None):
    ...
    train, val = split_data(file2idx,config=config)
    wc=count_labels(train,file2idx,config=config)
    # print(wc)
